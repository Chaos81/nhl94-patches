...
- H/Fs need to be adjusted for players based on 92-93 season fighting stats. Would like at least 1, maybe 2 fighters minimum for each team. (in progress)
- Reduce Minimum checks from 40. Compare to average checks/gm in a Classic season, and adjust.
- Add RAM locations to keep track of per player ChksF (would need to patch FallDown routine). Player checks would be cleared when a fight happens. This is to prevent
    another fight from the same player when they get out of the box.
...
- Set up variables for "Arcade" fighting option. 
