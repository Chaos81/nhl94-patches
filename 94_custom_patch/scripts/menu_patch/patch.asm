...
;--MACROS--
	include	scripts\menu_patch\scripts\macros.mac
...
;	org NewCodeAddress								        ; <-- Location in ROM to place new Menu Items + SubMenu Items + New Code - not needed since it is at end of fight_patch.asm
		include	scripts\menu_patch\scripts\menu\menuitems.asm		; <-- Main Menu Items
		include scripts\menu_patch\scripts\menu\submenuitems.asm	; <-- Sub Menu Items
		include scripts\menu_patch\scripts\menu\menulengths.asm		; <-- Main Menu Lengths
		include scripts\menu_patch\scripts\menu\menulengths2.asm	; <-- Main Menu Lengths for Fourway Play (MultiTap)
		include scripts\menu_patch\scripts\menu\writenewmenuram.asm	; <-- Code to Write New Menu RAM Addresses
		include scripts\menu_patch\scripts\menu\readnewmenuram.asm	; <-- Code to Read New Menu RAM Addresses				
;		include scripts\menu_patch\scripts\fighting\fighting.asm	; <-- Code to add the Fighting (not needed, added to end of fight_patch code)
		
; Hijack Code needs to be after the new code so it doesn't mess up assembler org values
		include scripts\menu_patch\scripts\menu\menuhijack.asm		; <-- Code to Hijack Old Menu Items and SubMenu Items		
