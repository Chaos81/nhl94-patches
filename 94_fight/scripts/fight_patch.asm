...
;   Version 1.0  - Official release, add change name of ROM and serial #, add code for new splash screen in 30 team ROM location, expand the ROM to 3MB included, add default to fight setting
;   Version 1.01 - Fix bug with shootout mode attribute list display

;--MACROS--
	include	scripts\macros.mac

;--Add the script to expand to 3MB - uses temp.bin in the rom folder
    include scripts\94_to_3MB.asm

;--Change name of ROM and serial #
    include scripts\chg_header.asm

;--Remove Checksum Code--
	include	scripts\patch_checksum.asm      ; Patches Checksum jmp in ROM

;--New Title Screen--
    include scripts\title_patch.asm

;--New Credits--
    include scripts\credits_patch.asm
...
defmenuPatch    equ $17C86      ; Address to patch in a default fight setting
...
; Default Fight setting (0 = Off, 1 = On, 2 = On, Arcade)
DefFight        equ 1                   ; Default will be 1 (On)
...
ChkFightValue:
; check to see if there should be a fight
...
loadDefFight:
; Set the default Fight option

.loop:
    move.w  (a1)+,(a0)+             ; finish setting the original option defaults
    dbf     d0,.loop

.chkdef:
    clr.w   d0
    move.w  #DefFight,d0
    cmpi.w  #2,d0                   ; make sure it is set to 2 or below
    ble.w   .setdf                  ; branch if so
    moveq   #1,d0                   ; set to 1 if not
    
.setdf:
    move.w  d0,(OptFight).w  ; Set default fight option
.ex:
    rts

; end of function loadDefFight



;------ Include sprite_patch.asm ------
    include scripts\sprite_patch.asm            ; Add updated sprites and animations, move and update tables associated with sprites and animations


NewAttribList                                   ; insert new attribute list from NHLPA93
    incbin data\93_Tables\AttribHdr.bin
...
NewCodeAddress
    include scripts\menu_patch\patch.asm
