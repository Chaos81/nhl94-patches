...
- Add RAM locations to keep track of per player ChksF (would need to patch FallDown routine). Player checks would be cleared when a fight happens. This would prevent another fight from the same player when they get out of the box. (Not needed, don't think this is an issue)
...
- Adjust arcade settings slightly. (done)
...
- Retrieve frame offset table.
- Retrieve frame data, write to a new file.
- Retrieve Hot Spot table.
